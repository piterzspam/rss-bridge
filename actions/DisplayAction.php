...
class DisplayAction extends ActionAbstract
{
	...
	public function execute()
	{
		...
		$format = $this->userData['format'] or returnClientError('You must specify a format!');
		...
		$noproxy = array_key_exists('_noproxy', $this->userData) && filter_var($this->userData['_noproxy'], FILTER_VALIDATE_BOOLEAN);
		...
		$bridge_params = array_diff_key($this->userData, array_fill_keys(array(
			'action',
			'bridge',
			'format',
			'_noproxy',
		...
		$cache_params = array_diff_key($this->userData, array_fill_keys(array(
			'action',
			'format',
			'_noproxy',
			'_cache_timeout',
		...
		$cache->setScope('');
		$cache->purgeCache(86400 * 14); // 24 hours * 14
			...
			catch(Error $e)
			{
				...
				if (logBridgeError($bridge::NAME, $e->getCode()) >= Configuration::getConfig('error', 'report_limit'))
				{
					if (Configuration::getConfig('error', 'output') === 'feed')
					{
						...
						if ($e->getCode() === 0)
						{
							$item->setTitle('Bridge encountered an unexpected situation! (' . $this->userData['_error_time'] . ')');
						}
						else
						{
							$item->setTitle('Bridge returned error ' . $e->getCode() . '! (' . $this->userData['_error_time'] . ')');
						}
